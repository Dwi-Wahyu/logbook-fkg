...
export function MahasiswaTable({ promises }: TableProps) {
  ...
  const columns: ColumnDef<ColumnType>[] = [
    ...
    },
    {
      header: "Angkatan",
      cell: ({ row }) => {
        return <span>{row.original.mahasiswa?.angkatan || "-"}</span>;
      },
    },
  ];
  ...
  async function handleDelete() {
    setLoading(true);
    const request = await deleteMahasiswa(selectedId);
  ...
  return (
    <div>
      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Anda Yakin Menghapus Mahasiswa?</AlertDialogTitle>
            <AlertDialogDescription>
